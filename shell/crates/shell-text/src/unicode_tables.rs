//! Unicode tables for the tokenizer. GENERATED -- do not edit.
//!
//! Produced by `tools/gen_unicode_tables.py`; run that script to regenerate.
//! Source: Python `unicodedata`, Unicode version 13.0.0.
//!
//! Why these exist: the Qwen2 pre-tokenizer splits on the general categories
//! `\p{L}`, `\p{M}` and `\p{N}`, and `char::is_alphabetic()` is not `\p{L}` --
//! it is the Alphabetic property, which additionally covers `Nl` and part of
//! `Mn`. The NFC tables are here for the same reason: neither category
//! membership nor canonical composition is available in `core`.

/// Inclusive ranges of General_Category L (all subcategories).
pub static LETTER_RANGES: [(u32, u32); 622] = [
    (0x41, 0x5A),
    (0x61, 0x7A),
    (0xAA, 0xAA),
    (0xB5, 0xB5),
...
/// Inclusive ranges of General_Category M.
pub static MARK_RANGES: [(u32, u32); 290] = [
    (0x300, 0x36F),
    (0x483, 0x489),
    (0x591, 0x5BD),
    (0x5BF, 0x5BF),
...
/// Inclusive ranges of General_Category N.
pub static NUMBER_RANGES: [(u32, u32); 133] = [
    (0x30, 0x39),
    (0xB2, 0xB3),
    (0xB9, 0xB9),
    (0xBC, 0xBE),
...
/// One-step canonical decomposition to a single codepoint. Excludes Hangul, which is algorithmic.
pub static DECOMPOSE_1: [(u32, u32); 1035] = [
    (0x340, 0x300),
    (0x341, 0x301),
    (0x343, 0x313),
    (0x374, 0x2B9),
...
/// One-step canonical decomposition to a pair, sorted by the first field.
pub static DECOMPOSE_2: [(u32, u32, u32); 1026] = [
    (0xC0, 0x41, 0x300),
    (0xC1, 0x41, 0x301),
    (0xC2, 0x41, 0x302),
    (0xC3, 0x41, 0x303),
...
/// Canonical combining class, for codepoints where it is non-zero.
pub static CCC: [(u32, u32); 872] = [
    (0x300, 0xE6),
    (0x301, 0xE6),
    (0x302, 0xE6),
    (0x303, 0xE6),
...
/// Canonical composition pairs, packed as `(a << 21) | b` so the table is a
/// flat sorted `u64` array. Composition exclusions are already applied.
pub static COMPOSE: [(u64, u32); 941] = [
    (0x7800338, 0x226E),
    (0x7A00338, 0x2260),
    (0x7C00338, 0x226F),
    (0x8200300, 0xC0),
